...
function add_sources(data, message_id) {
    ...
    if (data.citations) {
        const links = data.citations.map((citation, index) => {
            return `<p>[${index+1}] <a target="_blank" href="${citation.source || citation}">${(citation.excerpts.join('<br>\n') || citation).replace("https://www.", "").replace("https://", "")}</a></p>`;
        }).join("");
        blockquote.innerHTML = links;
    }
    if (data.sources) {
...
const load_conversation = async (conversation, append = false) => {
    ...
    messages.forEach((item, i) => {
        ...
        buffer = merge_messages(buffer, filter_message_content(item.content));
                ...
                <div class="content">
                    ...
                    <div class="count">
                        ${countTokensEnabled ? count_words_and_tokens(
                            ...
                            item.usage?.pollen_cost || item.usage?.market_cost || item.usage?.estimated_cost
                        ) : ""}
                        ${add_buttons.join("")}
                    </div>
                </div>
...
const load_conversations = async () => {
    ...
    conversations.forEach((conversation) => {
        ...
        convo.innerHTML = `
            <a class="left" href="#${conversation.id}">
...
const say_hello = async () => {
    const tokens = framework.translate(`Hello! How can I assist you today?`).split(" ").map((token) => token + " ");
